from HRR.with_tensorflow import normal, projection, binding, unbinding, cosine_similarity
from HRR.with_tensorflow import projection_2d, binding_2d, unbinding_2d
...
x = projection(normal(shape=(batch, features), seed=0))
y = projection(normal(shape=(batch, features), seed=1))
w = projection(normal(shape=(batch, features), seed=4))
z = projection(normal(shape=(batch, features), seed=5))
...
score = cosine_similarity(y, y_, axis=-1)
print('y:', y[0])
print('y_prime:', y_[0])
print('score:', score[0])
...
score = cosine_similarity(y, y_, axis=(2, 3))
print('y:', y[0][0][0])
